    ...
    <div className="scrollable-div">
      ...
      {data.map((name) => {
        ...
        if (
          ...
          return (
            <button
              style={{ color: name.table_color }}
              className="tableNameButton"
              value={name.table_name}
              onClick={(e) => {
                props.setState(e.target.value);
                setLastSelected(name.table_name);
              }}
            >
              {name.table_name}
              {lastSelected === name.table_name && ( // if last selected table is name of currently selected table. SHOW THE DOG !!!
